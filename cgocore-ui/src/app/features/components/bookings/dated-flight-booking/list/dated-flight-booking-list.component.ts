...
displayedColumns: string[] = ['datedFlightCompanyIataCode','datedFlightFlightNumber','datedFlightDepartureDate','bookingAirWaybillAirWaybillNumber','numberOfPieces','grossWeight','grossWeightUnitOfMeasureCode','grossVolume','grossVolumeUnitOfMeasureCode','Actions'];
filter: FormGroup;

constructor(private service:DatedFlightBookingRestClient, private formBuilder: FormBuilder, private dialog: MatDialog, private notifications: NotificationService) { }
ngOnInit(): void {
...
bookingAirWaybillAirWaybillNumber:[null]})
this.reset();
}
...
case 'bookingAirWaybillAirWaybillNumber': this.scrollForm.sorting.bookingAirWaybillAirWaybillNumberOrderType = StringUtils.emptyToNull(this.sort.direction.toUpperCase());break;
...
this.filter.controls['bookingAirWaybillAirWaybillNumber'].valueChanges.subscribe(value=>{
this.scrollForm.filter.bookingAirWaybillAirWaybillNumber=value;
this.refresh();
});
}
