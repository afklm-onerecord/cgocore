...
scrollForm: ScrollForm<MetricFilter, MetricSorting> = new ScrollForm();
scrollView: ScrollView<MetricBasicView> = new ScrollView();
dataSource:MatTableDataSource<MetricBasicView>;
...
constructor(private service:MetricRestClient, private formBuilder: FormBuilder, private dialog: MatDialog, private notifications: NotificationService) { }
ngOnInit(): void {
...
(event) => {
this.scrollForm.sorting = new MetricSorting();
...
this.scrollForm.filter = new MetricFilter();
this.scrollForm.sorting = new MetricSorting();
...
let ref = this.dialog.open(MetricModalComponent);
