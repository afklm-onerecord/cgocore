...
export class UnitOfMeasureListComponent implements OnInit, AfterViewInit {

scrollForm: ScrollForm<UnitOfMeasureFilter, UnitOfMeasureSorting> = new ScrollForm();
scrollView: ScrollView<UnitOfMeasureBasicView> = new ScrollView();
dataSource:MatTableDataSource<UnitOfMeasureBasicView>;
@ViewChild(MatPaginator) paginator: MatPaginator;
@ViewChild(MatSort) sort: MatSort
pageSizeOptions: number[] = [10, 20, 50, 100];
displayedColumns: string[] = ['code','label','metricCode','Actions'];
filter: FormGroup;

constructor(private service:UnitOfMeasureRestClient, private formBuilder: FormBuilder, private dialog: MatDialog, private notifications: NotificationService) { }
ngOnInit(): void {
...
(event) => {
this.scrollForm.sorting = new UnitOfMeasureSorting();
...
case 'metricCode': this.scrollForm.sorting.metricCodeOrderType = StringUtils.emptyToNull(this.sort.direction.toUpperCase());break;
}
this.refresh();
});
...
this.filter.controls['metricCode'].valueChanges.subscribe(value=>{
this.scrollForm.filter.metricCode=value;
this.refresh();
});
}
...
this.scrollForm.filter = new UnitOfMeasureFilter();
this.scrollForm.sorting = new UnitOfMeasureSorting();
...
let ref = this.dialog.open(UnitOfMeasureModalComponent);
